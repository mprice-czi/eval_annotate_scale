...
from src.bazel_utils import ensure_output_directory, resolve_workspace_path
...
def create_demo_processed_passages(max_passages: int = 3) -> List[ProcessedPassage]:
    ...
    for _, row in df_clean.iterrows():
        ...
        for i, segment_text in enumerate(segments):
            ...
            focus_words = [
                w.strip(".,!?") for w in words if len(w) > 6 and w.isalpha()
            ][:5]
...
def create_demo_pairs(
    processed_passages: List[ProcessedPassage], target_pairs: int = 2
) -> List[PassagePair]:
...
def main():
    ...
    for passage in processed_passages:
        print(
            f"   - {passage.segment_id}: {passage.complexity_estimate} "
    ...
    for pair in pairs:
        print(
            f"   - {pair.pair_id}: {pair.passage_a.segment_id} + {pair.passage_b.segment_id} "
