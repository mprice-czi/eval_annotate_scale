...
def read_key_file() -> str:
    ...
    try:
        ...
        print(
            f"✅ Successfully read API key from 'key' file ({len(api_key)} characters)"
        )
        return api_key
...
def verify_bazelrc_import() -> None:
    ...
    try:
        ...
        if (
            ".bazelrc.local" in content
            or "try-import %workspace%/.bazelrc.local" in content
        ):
...
def main():
    ...
    print(
        "   bazel run //scripts:intelligent_preprocessing -- --output data/output.json"
    )
